 ...
 * @Last Modified time: 2017-10-24 14:29:08
 */
'use strict';
...
module.exports = function (Patient) {
    ...
    function UpdateUserLBS(location) {
        ...
        var bsSQL = "update hh_publicuser set location_y = '" + location.longitude[0] + "', location_x = '" + location.latitude[0] + "' where openid = '" + location.fromusername[0] + "'";
        ...
        var job = schedule.scheduleJob(rule, function () {
            ...
            if (_curTime == '04') {
                ...
                if (_curMinute == '30') {
                    ...
                    var bsSQL = "insert into hh_usersportdata_history select * from hh_usersportdata where addtime <= DATE_ADD(now(),interval -15 day);delete from hh_usersportdata where addtime <= DATE_ADD(now(),interval -7 day)";
                    DoSQL(bsSQL).then(function () {

                        bsSQL = "insert into hh_userwatchdata_history select * from hh_userwatchdata where addtime <= DATE_ADD(now(),interval -15 day);delete from hh_userwatchdata where addtime <= DATE_ADD(now(),interval -7 day)";
                        DoSQL(bsSQL).then(function () {
